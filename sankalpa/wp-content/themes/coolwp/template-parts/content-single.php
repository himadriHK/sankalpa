                ...
                if ( !(coolwp_get_option('hide_thumbnail')) ) {
                    if ( !(coolwp_get_option('hide_thumbnail_single')) ) {
                        if ( coolwp_get_option('thumbnail_link') == 'no' ) {
                            the_post_thumbnail('coolwp-featured-image', array('class' => 'coolwp-post-thumbnail-single'));
                        } else { ?>
                            <a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php /* translators: %s: post title. */ echo esc_attr( sprintf( __( 'Permanent Link to %s', 'coolwp' ), the_title_attribute( 'echo=0' ) ) ); ?>"><?php the_post_thumbnail('coolwp-featured-image', array('class' => 'coolwp-post-thumbnail-single')); ?></a>
                <?php   }
                    }
                }
            }
