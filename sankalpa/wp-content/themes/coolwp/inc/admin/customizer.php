...
function coolwp_register_theme_customizer( $wp_customize ) {

    if(method_exists('WP_Customize_Manager', 'add_panel')):
    $wp_customize->add_panel('coolwp_main_options_panel', array( 'title' => esc_html__('Theme Options', 'coolwp'), 'priority' => 10, ));
    endif;
    
    $wp_customize->get_section( 'title_tagline' )->panel = 'coolwp_main_options_panel';
    $wp_customize->get_section( 'title_tagline' )->priority = 20;
    $wp_customize->get_section( 'colors' )->panel = 'coolwp_main_options_panel';
    ...
    coolwp_getting_started($wp_customize);
    coolwp_header_options($wp_customize);
    coolwp_slider_options($wp_customize);
    coolwp_post_options($wp_customize);
    coolwp_social_profiles($wp_customize);
    coolwp_footer_options($wp_customize);
    coolwp_other_options($wp_customize);
    coolwp_recomm_plugin_options($wp_customize);
    coolwp_upgrade_to_pro($wp_customize);

}

add_action( 'customize_register', 'coolwp_register_theme_customizer' );

function coolwp_customizer_js_scripts() {
    wp_enqueue_script('coolwp-theme-customizer-js', get_template_directory_uri() . '/inc/admin/js/customizer.js', array( 'jquery', 'customize-preview' ), NULL, true);
}
add_action( 'customize_preview_init', 'coolwp_customizer_js_scripts' );