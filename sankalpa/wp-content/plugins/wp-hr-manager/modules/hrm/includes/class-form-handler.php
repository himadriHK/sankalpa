...
class Form_Handler {
    ...
    public function leave_policies() {
        ...
        if (isset($_POST['action']) && $_POST['action'] == 'trash') {
            ...
            if (isset($_POST['policy_id'])) {
                wphr_hr_leave_policy_delete($_POST['policy_id']);
            }
        }
            ...
            switch ($action) {
                ...
                case 'delete' :
                    ...
                    if (isset($_GET['employee_id']) && !empty($_GET['employee_id'])) {
                        wphr_employee_delete($_GET['employee_id'], false);
                    }
                ...
                case 'permanent_delete' :
                    if (isset($_GET['employee_id']) && !empty($_GET['employee_id'])) {
                        wphr_employee_delete($_GET['employee_id'], true);
                    }
                ...
                case 'restore' :
                    if (isset($_GET['employee_id']) && !empty($_GET['employee_id'])) {
                        wphr_employee_restore($_GET['employee_id']);
                    }
            ...
            switch ($action) {
                ...
                case 'designation_delete' :
                    ...
                    if (isset($_GET['desig']) && !empty($_GET['desig'])) {
                        $not_deleted_item = wphr_hr_delete_designation($_GET['desig']);
                    }
    ...
    public function leave_entitlement() {
        ...
        if (!wp_verify_nonce($_POST['_wpnonce'], 'wphr-hr-leave-assign')) {
    ...
    public function leave_request_status_change() {
        ...
        switch ($action) {
            case 'delete':
                \WPHR\HR_MANAGER\HRM\Models\Leave_request::find($_GET['id'])->delete();
                break;
            ...
            case 'archive':
                $status = $_GET['alllist'] ? 'all' : 1;
                wphr_hr_archive_unarchive_approved_leaves($request_id);
                break;

            case 'unarchive':
                $status = $_GET['alllist'] ? 'all' : 4;
                wphr_hr_archive_unarchive_approved_leaves($request_id, 0);
                break;
        }
    ...
    public function handle_employee_status_update() {
        ...
        if ($_POST['employee_status'] == 'terminated') {
            \WPHR\HR_MANAGER\HRM\Models\Employee::where('user_id', '=', $_POST['user_id'])->update(['status' => $_POST['employee_status'], 'termination_date' => current_time('mysql')]);
        } else {
            \WPHR\HR_MANAGER\HRM\Models\Employee::where('user_id', '=', $_POST['user_id'])->update(['status' => $_POST['employee_status'], 'termination_date' => '']);
        }
    ...
    public function employee_permission() {
        ...
        if (!wp_verify_nonce($_POST['_wpnonce'], 'wp-wphr-hr-employee-permission-nonce')) {
            return;
        }
