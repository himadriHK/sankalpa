...
function wphr_hrm_is_leave_recored_exist_between_dateTime( $start_dateTime, $end_dateTime, $user_id )
{
    ...
    $query = "select * from `wp_wphr_hr_leave_requests` where `status` in (1, 2) and `user_id` = {$user_id} and ( `start_date` BETWEEN '{$start_dateTime}' AND '{$end_dateTime}' OR  `end_date` BETWEEN '{$start_dateTime}' AND '{$end_dateTime}' )";
...
function wphr_hr_leave_insert_entitlement( $args = array() )
{
    ...
    $policy = $wpdb->get_row( "SELECT * FROM `{$wpdb->prefix}wphr_hr_leave_policies` WHERE `id` = {$policy_id}" );
...
function wphr_hr_holiday_filter_param( $holiday, $args )
{
    ...
    if ( $args_s && !empty($args['s']) ) {
        ...
        $holiday = $holiday->Where( function ( $q ) {
            $q->where( 'description', 'LIKE', '%' . $_GET['s'] . '%' )->orwhere( 'title', 'LIKE', '%' . $_GET['s'] . '%' );
        } );
    }
...
{
    $policies = wphr_hr_leave_get_policies();
    $dropdown = array();
...
function wphr_hr_leave_get_requests_count( $args = array() )
{
    ...
    $user_id_in = '';
    if ( isset( $args['user_id_in'] ) ) {
        $user_id_in = 'AND user_id IN (' . implode( ',', $args['user_id_in'] ) . ') ';
    }
    foreach ( $statuses as $status => $label ) {
        ...
        if ( $status == 4 ) {
            $sql2 = "SELECT COUNT(id) as num FROM {$wpdb->prefix}wphr_hr_leave_requests WHERE status = 1 AND is_archived = 1 {$user_id_in} GROUP BY status;";
            $archived_cnt = $wpdb->get_row( $sql2 );
            if ( $archived_cnt ) {
                $counts[$status] = array(
            ...
            $sql3 = "SELECT COUNT(id) as num FROM {$wpdb->prefix}wphr_hr_leave_requests WHERE status = 1 AND is_archived = 0 {$user_id_in} GROUP BY status;";
            $approved_cnt = $wpdb->get_row( $sql3 );
            if ( $approved_cnt ) {
                $counts[$status] = array(
    ...
    if ( false === $results ) {
        $sql = "SELECT status, COUNT(id) as num FROM {$wpdb->prefix}wphr_hr_leave_requests WHERE status != 0 {$user_id_in} GROUP BY status;";
...
function wphr_hr_leave_count_entitlements( $args = array() )
{
    ...
    $args = wp_parse_args( $args, $defaults );
    return \WPHR\HR_MANAGER\HRM\Models\Leave_Entitlement::where( 'from_date', '>=', $args['from_date'] )->where( 'to_date', '<=', $args['to_date'] )->count();
}
...
{
    $leave_recored = \WPHR\HR_MANAGER\HRM\Models\Leave_request::where( 'user_id', '=', $user_id )->where( 'policy_id', '=', $policy_id )->get()->toArray();
...
function wphr_hr_leave_get_balance( $user_id, $leave_date = false )
{
    ...
    $query .= " LEFT JOIN {$wpdb->prefix}wphr_hr_leave_entitlements as en on (req.user_id = en.user_id and req.policy_id = en.policy_id and en.from_date >= '{$financial_start_date}' )";
...
)
{
    ...
    wphr_hr_leave_insert_entitlement( $policy_arg );
