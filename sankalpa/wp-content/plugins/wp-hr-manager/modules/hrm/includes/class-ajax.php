...
class Ajax_Handler {
    ...
    function holiday_remove() {
        ...
        $holiday = wphr_hr_delete_holidays(array('id' => intval($_POST['id'])));
        $this->send_success();
    }
    ...
    function import_ical() {
        ...
        if (isset($_POST['location_id']) && $_POST['location_id'] != 0 && !empty($_POST['location_id'])) {
    ...
    public function employee_remove_history() {
        ...
        $id = isset($_POST['id']) ? intval($_POST['id']) : 0;
        $query = "SELECT module, user_id FROM {$wpdb->prefix}wphr_hr_employee_history WHERE id=" . $id;
    ...
    public function employee_update_performance() {
        ...
        if (isset($_POST['employee_id']) && $_POST['employee_id'] && !current_user_can('wphr_edit_employee', $_POST['employee_id'])) {
            $this->send_error(__('You do not have sufficient permissions to do this action', 'wphr'));
        }
    ...
    public function leave_request_dates() {
        ...
        $employee_id = isset($_POST['employee_id']) && $_POST['employee_id'] > 0 ? intval($_POST['employee_id']) : false;
		...
		*/
		...
		$financial_start_date = date('M-Y', strtotime( $financial_year_dates['start'] ) );
		$financial_end_date = date('M-Y', strtotime( $financial_year_dates['end'] ) );
        ...
        if ($available < 0) {
            $content = ( $available_arr['balance_string'] ) ? '<span class=\'description red\'>' . $available_arr['balance_string'] . ' are available ( '.$financial_start_date.' to '.$financial_end_date.' )</span>' : '-';
        } elseif ($available > 0) {
            $content = ( $available_arr['balance_string'] ) ? '<span class=\'description green\'>' . $available_arr['balance_string'] . ' are available ( '.$financial_start_date.' to '.$financial_end_date.' )</span>' : '-';
        } else {
    ...
    public function leave_request_times() {
        ...
        $start_time = isset($_POST['fromTime']) ? strtotime($_POST['fromTime']) : '';
        $end_time = isset($_POST['toTime']) ? strtotime($_POST['toTime']) : '';
    ...
    public function leave_available_days() {
		...
		$start = date('M-Y', strtotime( $financial_year_dates['start'] ) );
		$end = date('M-Y', strtotime( $financial_year_dates['end'] ) );
        if ($available < 0) {
            $content = ( $available_arr['balance_string'] ) ? '<span class="description red">' . $available_arr['balance_string'] . ' are available ( '.$start.' to '.$end.' )</span>' : '-';
        } elseif ($available > 0) {
            $content = ( $available_arr['balance_string'] ) ? '<span class="description green">' . $available_arr['balance_string'] . ' are available ( '.$start.' to '.$end.' )</span>' : '-';
        } else {
            $leave_policy_day = \WPHR\HR_MANAGER\HRM\Models\Leave_Policies::select('value')->where('id', $policy_id)->pluck('value');
            $content = sprintf('<span class="description">%d %s ( %s to %s )</span>', number_format_i18n($leave_policy_day), __('days are available', 'wphr'), $start, $end);
        }
    ...
    public function leave_request() {
        ...
        $date_array = array();
        $working_hours = get_employee_working_hours($_POST['employee_id']);
        $length_hours = $working_hours;
        $from_time = !empty($_POST['from_time']) ? strtotime($_POST['from_time']) : 0;
        $to_time = !empty($_POST['to_time']) ? strtotime($_POST['to_time']) : 0;
        ...
        $date_array = array();
        if ($_POST['leave_from'] != $_POST['leave_to']) {
            $day_wise_from_times = $_POST['day_wise_from_times'];
            $day_wise_to_times = $_POST['day_wise_to_times'];
        ...
        if ($from_time) {
            $start_date = isset($_POST['leave_from']) ? sanitize_text_field($_POST['leave_from'] . ' ' . date('H:i:s', $from_time)) : date_i18n('Y-m-d') . ' ' . date('H:i:s', $from_time);
        }
        if ($to_time) {
            $end_date = isset($_POST['leave_to']) ? sanitize_text_field($_POST['leave_to'] . ' ' . date('H:i:s', $to_time)) : date_i18n('Y-m-d') . ' ' . date('H:i:s', $to_time);
        }
