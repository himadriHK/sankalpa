...
function wphr_hr_employee_create( $args = array() ) {
    ...
    $employee_table_data = array(
		...
		'leave_year' => $data['work']['leave_year'],
		'apply_leave_year' =>  isset( $data['work']['apply_leave_year'] ) ? $data['work']['apply_leave_year'] : '',
    );
    ...
    // update the wphr table
  	$employee_row_id = $wpdb->get_var( 'SELECT id FROM '.$wpdb->prefix . 'wphr_hr_employees WHERE user_id = '.$user_id );
	if( $employee_row_id ){
