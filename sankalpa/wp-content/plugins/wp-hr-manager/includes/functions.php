...
function wphr_process_actions() {
    if ( isset( $_REQUEST['wphr-action'] ) ) {
        do_action( 'wphr_action_' . $_REQUEST['wphr-action'], $_REQUEST );
    }  
}
...
function wphr_process_import_export() {
    if ( ! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'wphr-import-export-nonce' ) ) {
        return;
    }
            ...
            foreach ( $csv_data as $line ){
                ...
                if ( ( $type == 'contact' || $type == 'company' ) && $is_crm_activated ) {
                    ...
                    if ( is_wp_error( $people ) ) {
                        ...
                        $contact_owner = isset( $_POST['contact_owner'] ) ? absint( $_POST['contact_owner'] ) : wphr_crm_get_default_contact_owner();
...
function wphr_importer_notices() {
    if ( ! isset( $_REQUEST['page'] ) || $_REQUEST['page'] != 'wphr-tools' || ! isset( $_REQUEST['tab'] ) || $_REQUEST['tab'] != 'import' ) {
        return;
    }
    ...
    if ( isset( $_REQUEST['imported'] ) ) {
        if ( intval( $_REQUEST['imported'] ) == 0 ) {
            ...
            $message = sprintf( __( '%s items successfully imported.', 'wphr' ), number_format_i18n( $_REQUEST['imported'] ) );
...
function wphr_get_financial_year_dates_by_user( $user_id, $leave_date = false ){
	...
	if( !$employee->get_apply_leave_year() ){
	    ...
	    $office_financial_year_start = $wpdb->get_var( "SELECT office_financial_year_start FROM `{$wpdb->prefix}wphr_company_locations` WHERE id = $user_location" );
	}
    /**
...
function get_user_leave_years( $user_id ){
	global $wpdb;
	$sql = "SELECT DISTINCT YEAR(start_date) AS date FROM `{$wpdb->prefix}wphr_hr_leave_requests`  WHERE user_id = $user_id AND status = 1";
	$start_date_list = $wpdb->get_col( $sql );
	$sql = "SELECT DISTINCT YEAR(end_date) AS date FROM `{$wpdb->prefix}wphr_hr_leave_requests`  WHERE user_id = $user_id AND status = 1";
